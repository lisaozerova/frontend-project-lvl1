...
const playEven = (name) => {
  ...
  for (round; round < gameNumbers.length; round += 1) {
    const number = gameNumbers[round];
    const userAnswer = readlineSync.question(`${GamePhrase.QUESTION} ${number}\r\n${GamePhrase.ANSWER}`)
      .trim()
      .toLowerCase();

    const correctAnswer = getCorrectAnswerPhrase(isNumberParity(number));
